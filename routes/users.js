...
module.exports = function(app) {
		...
		/*
         * Upload calendar Event file
         */
        app.post('/uploadCalendarEventFile', multipartMiddleware, function(req, res) {
		
			var file = req.files.file;
            var fs = require('fs');
            var savePath = './public/calendarupload/';
			fs.readFile(file.path, function(err, data) {
				if (err) {
					res.send(err);
				} else {
					var fileExt = (file.name).substr((file.name).lastIndexOf('.') + 1);
					var newfilename =  Math.random().toString(36).substr(2, 9)+"."+fileExt;
					//fs.writeFile(savePath + file.name, data, function(err) {
					fs.writeFile(savePath + newfilename, data, function(err) {
						if (err) {
							res.send(err);
						} else {
